...
def xgb_predict(request):
    ...
    input_data = input_data_raw.filter(si__gt=0)
    # QuerySet 객체에서 값 꺼내기
    ...
    if input_data.count() > 0:
        ...
        max_value=float(predict_value.max())
        predict_value=predict_value.tolist()
        
        # 예측값에 구멍이 있는지 없는지 확인하여 구멍은 0으로 채워주기
        for idx, val in enumerate(input_data_raw_list):
            if val in input_data_list:
                value[idx]=float(predict_value.pop(0))
    ...
    result = [{"LOC":location,"TM":time, "value":val} for time, val in zip(timeline,value)]
    result.append({"Max":max_value})
    # json 형태로 결과 리턴
